...
/**
 * MF_PHPExcel_Writer_Excel5_Workbook
 *
 * @category   MF_PHPExcel
 * @package    MF_PHPExcel_Writer_Excel5
 * @copyright  Copyright (c) 2006 - 2010 MF_PHPExcel (http://www.codeplex.com/MF_PHPExcel)
 */
class MF_PHPExcel_Writer_Excel5_Workbook extends MF_PHPExcel_Writer_Excel5_BIFFwriter
{
	/**
	 * Formula parser
	 *
	...
	/**
	 * XF Writers
	 * @var MF_PHPExcel_Writer_Excel5_Xf[]
	 */
	 ...
	 * Fonts writers
	 *
	 * @var MF_PHPExcel_Writer_Excel5_Font[]
	 */
	 ...
	 * Class constructor
	 *
	 * @param MF_PHPExcel $MF_PHPExcel The Workbook
	 ...
	 */
	public function __construct(MF_PHPExcel $MF_PHPExcel = null, $BIFF_version = 0x0600,
												&$str_total,
								...
								)
	{
		...
		$countSheets = $MF_PHPExcel->getSheetCount();
		for ($i = 0; $i < $countSheets; ++$i) {
			$phpSheet = $MF_PHPExcel->getSheet($i);
	...
	{
		$xfWriter = new MF_PHPExcel_Writer_Excel5_Xf($style);
		...
		if (isset($this->_addedFonts[$fontHashCode])) {
			...
			$fontWriter = new MF_PHPExcel_Writer_Excel5_Font($style->getFont());
	...
	public function writeWorkbook($pWorksheetSizes = null)
	{
		...
		$total_worksheets = $this->_MF_PHPExcel->getSheetCount();
		...
		for ($i = 0; $i < $total_worksheets; ++$i) {
			$this->_writeBoundsheet($this->_MF_PHPExcel->getSheet($i), $this->_worksheetOffsets[$i]);
		}
	...
	function _calcSheetOffsets()
	{
		...
		$total_worksheets = count($this->_MF_PHPExcel->getAllSheets());
		foreach ($this->_MF_PHPExcel->getWorksheetIterator() as $sheet) {
			if ($this->_BIFF_version == 0x0600) {
				$offset += $boundsheet_length + strlen(MF_PHPExcel_Shared_String::UTF8toBIFF8UnicodeShort($sheet->getTitle()));
			} else {
	...
	{
		$countSheets = $this->_MF_PHPExcel->getSheetCount();
		// Create EXTERNCOUNT with number of worksheets
		$this->_writeExterncount($countSheets);

		// Create EXTERNSHEET for each worksheet
		for ($i = 0; $i < $countSheets; ++$i) {
			$this->_writeExternsheet($MF_PHPExcel->getSheet($i)->getTitle());
		}
	}
	...
	private function _writeNames()
	{
		...
		for ($i = 0; $i < $total_worksheets; ++$i) {
			$sheetSetup = $this->_MF_PHPExcel->getSheet($i)->getPageSetup();
			// Write a Name record if the print area has been defined
			if ($sheetSetup->isPrintAreaSet()) {
				// Print area
				$printArea = MF_PHPExcel_Cell::splitRange($sheetSetup->getPrintArea());
				$printArea = $printArea[0];
				$printArea[0] = MF_PHPExcel_Cell::coordinateFromString($printArea[0]);
				$printArea[1] = MF_PHPExcel_Cell::coordinateFromString($printArea[1]);

				$print_rowmin = $printArea[0][1] - 1;
				$print_rowmax = $printArea[1][1] - 1;
				$print_colmin = MF_PHPExcel_Cell::columnIndexFromString($printArea[0][0]) - 1;
				$print_colmax = MF_PHPExcel_Cell::columnIndexFromString($printArea[1][0]) - 1;
		...
		for ($i = 0; $i < $total_worksheets; ++$i) {
			...
			if ($sheetSetup->isColumnsToRepeatAtLeftSet() && $sheetSetup->isRowsToRepeatAtTopSet()) {
				...
				$colmin = MF_PHPExcel_Cell::columnIndexFromString($repeat[0]) - 1;
				$colmax = MF_PHPExcel_Cell::columnIndexFromString($repeat[1]) - 1;
	...
	private function _writeAllDefinedNamesBiff8()
	{
		...
		// Named ranges
		if (count($this->_MF_PHPExcel->getNamedRanges()) > 0) {
			// Loop named ranges
			$namedRanges = $this->_MF_PHPExcel->getNamedRanges();
			foreach ($namedRanges as $namedRange) {

				// Create absolute coordinate
				$range = MF_PHPExcel_Cell::splitRange($namedRange->getRange());
				for ($i = 0; $i < count($range); $i++) {
					$range[$i][0] = '\'' . str_replace("'", "''", $namedRange->getWorksheet()->getTitle()) . '\'!' . MF_PHPExcel_Cell::absoluteCoordinate($range[$i][0]);
					if (isset($range[$i][1])) {
						$range[$i][1] = MF_PHPExcel_Cell::absoluteCoordinate($range[$i][1]);
					}
				}
				$range = MF_PHPExcel_Cell::buildRange($range); // e.g. Sheet1!$A$1:$B$2
				...
				try {
					...
					if ($namedRange->getLocalOnly()) {
						// local scope
						$scope = $this->_MF_PHPExcel->getIndex($namedRange->getScope()) + 1;
					} else {
						// global scope
						$scope = 0;
					}
		...
		for ($i = 0; $i < $total_worksheets; ++$i) {
			...
			if ($sheetSetup->isPrintAreaSet()) {
				...
				for ($j = 0; $j < $countPrintArea; ++$j) {
					...
					$printAreaRect[0] = MF_PHPExcel_Cell::coordinateFromString($printAreaRect[0]);
					$printAreaRect[1] = MF_PHPExcel_Cell::coordinateFromString($printAreaRect[1]);

					$print_rowmin = $printAreaRect[0][1] - 1;
					$print_rowmax = $printAreaRect[1][1] - 1;
					$print_colmin = MF_PHPExcel_Cell::columnIndexFromString($printAreaRect[0][0]) - 1;
					$print_colmax = MF_PHPExcel_Cell::columnIndexFromString($printAreaRect[1][0]) - 1;
	...
	private function _writeDefinedNameBiff8($name, $formulaData, $sheetIndex = 0, $isBuiltIn = false)
	{
		...
		$nlen = MF_PHPExcel_Shared_String::CountCharacters($name);

		// name with stripped length field
		$name = substr(MF_PHPExcel_Shared_String::UTF8toBIFF8UnicodeLong($name), 2);
	...
	private function _writeWindow1()
	{
		...
		// not supported by MF_PHPExcel, so there is only one selected sheet, the active
		$ctabsel   = 1;       // Number of workbook tabs selected

		$wTabRatio = 0x0258;                 // Tab to scrollbar ratio

		// not supported by MF_PHPExcel, set to 0
		$itabFirst = 0;     // 1st displayed worksheet
		$itabCur   = $this->_MF_PHPExcel->getActiveSheetIndex();    // Active worksheet
	 ...
	 *
	 * @param MF_PHPExcel_Worksheet  $sheet Worksheet name
	...
	private function _writeBoundsheet($sheet, $offset)
	{
		...
		switch ($sheet->getSheetState()) {
			case MF_PHPExcel_Worksheet::SHEETSTATE_VISIBLE:	$ss = 0x00; break;
			case MF_PHPExcel_Worksheet::SHEETSTATE_HIDDEN:		$ss = 0x01; break;
			case MF_PHPExcel_Worksheet::SHEETSTATE_VERYHIDDEN:	$ss = 0x02; break;
		...
		if ($this->_BIFF_version == 0x0600) {
			...
			$data .= MF_PHPExcel_Shared_String::UTF8toBIFF8UnicodeShort($sheetname);
		} else {
	...
	private function _writeSupbookInternal()
	{
		...
		$header    = pack("vv", $record, $length);
		$data      = pack("vv", $this->_MF_PHPExcel->getSheetCount(), 0x0401);
	...
	private function _writeNumFormat($format, $ifmt)
	{
		...
		if ($this->_BIFF_version == 0x0600) {
			$numberFormatString = MF_PHPExcel_Shared_String::UTF8toBIFF8UnicodeLong($format);
	...
	private function _writeDatemode()
	{
		...
		$f1904     = (MF_PHPExcel_Shared_Date::getExcelCalendar() == MF_PHPExcel_Shared_Date::CALENDAR_MAC_1904) ?

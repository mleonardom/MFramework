 ...
 * @package    MF_PHPExcel_Writer_Excel5
...
/**
 * MF_PHPExcel_Writer_Excel5_Worksheet
 *
 * @category   MF_PHPExcel
 * @package    MF_PHPExcel_Writer_Excel5
 * @copyright  Copyright (c) 2006 - 2010 MF_PHPExcel (http://www.codeplex.com/MF_PHPExcel)
 */
class MF_PHPExcel_Writer_Excel5_Worksheet extends MF_PHPExcel_Writer_Excel5_BIFFwriter
{
	/**
	 * Formula parser
	 *
	 * @var MF_PHPExcel_Writer_Excel5_Parser
	 */
	private $_parser;
	 ...
	 * @param MF_PHPExcel_Worksheet $phpSheet
	 */
												...
												$parser, $preCalculateFormulas, $phpSheet)
	{
		...
		$this->_firstColumnIndex	= (count($col) > 0) ? MF_PHPExcel_Cell::columnIndexFromString(substr(min($col),1)) : 1;
		$this->_lastColumnIndex		= (count($col) > 0) ? MF_PHPExcel_Cell::columnIndexFromString(substr(max($col),1)) : 1;
	 ...
	 *
	 * @access public
	 * @see MF_PHPExcel_Writer_Excel5_Workbook::storeWorkbook()
	 */
	function close()
	{
		...
		for ($i = 0; $i < 256; ++$i) {
			...
			if ($this->_phpSheet->getDefaultColumnDimension()->getWidth() >= 0) {
				...
				$width = MF_PHPExcel_Shared_Font::getDefaultColumnWidthByFont($this->_phpSheet->getParent()->getDefaultStyle()->getFont());
			}

			$columnLetter = MF_PHPExcel_Cell::stringFromColumnIndex($i);
		...
		foreach ($this->_phpSheet->getCellCollection() as $cellID) {
			...
			$row = $cell->getRow() - 1;
			$column = MF_PHPExcel_Cell::columnIndexFromString($cell->getColumn()) - 1;
			...
			if ($cell->getValue() instanceof MF_PHPExcel_RichText) {
				...
				case MF_PHPExcel_Cell_DataType::TYPE_STRING:
		...
		if ($this->_BIFF_version == 0x0600) {
			foreach ($this->_phpSheet->getHyperLinkCollection() as $coordinate => $hyperlink) {
				list($column, $row) = MF_PHPExcel_Cell::coordinateFromString($coordinate);
				...
				$this->_writeUrl($row - 1, MF_PHPExcel_Cell::columnIndexFromString($column) - 1, $url);
			}
		}
	...
	private function _writeBIFF8CellRangeAddressFixed($range = 'A1')
	{
		...
		$firstCellCoordinates = MF_PHPExcel_Cell::coordinateFromString($firstCell); // e.g. array(0, 1)
		$lastCellCoordinates  = MF_PHPExcel_Cell::coordinateFromString($lastCell);  // e.g. array(1, 6)

		return(pack('vvvv',
			$firstCellCoordinates[1] - 1,
			$lastCellCoordinates[1] - 1,
			MF_PHPExcel_Cell::columnIndexFromString($firstCellCoordinates[0]) - 1,
			MF_PHPExcel_Cell::columnIndexFromString($lastCellCoordinates[0]) - 1
		));
	}
	...
	private function _writeNumber($row, $col, $num, $xfIndex)
	{
		...
		if (MF_PHPExcel_Writer_Excel5_BIFFwriter::getByteOrder()) { // if it's Big Endian
	...
	private function _writeLabelSst($row, $col, $str, $xfIndex)
	{
		...
		$str = MF_PHPExcel_Shared_String::UTF8toBIFF8UnicodeLong($str);
	...
	private function _writeFormula($row, $col, $formula, $xfIndex, $calculatedValue)
	{
		...
		if (isset($calculatedValue)) {
			...
			if (is_bool($calculatedValue)) {
				...
				if (array_key_exists($calculatedValue, MF_PHPExcel_Cell_DataType::getErrorCodes())) {
					// Error value
	...
	private function _writeStringRecord($stringValue)
	{
		...
		$data = MF_PHPExcel_Shared_String::UTF8toBIFF8UnicodeLong($stringValue);
	...
	function _writeUrlInternal($row1, $col1, $row2, $col2, $url)
	{
		...
		// character count
		$url_len = MF_PHPExcel_Shared_String::CountCharacters($url);
		$url_len = pack('V', $url_len);

		$url = MF_PHPExcel_Shared_String::ConvertEncoding($url, 'UTF-16LE', 'UTF-8');
	...
	private function _writeWindow2()
	{
		...
		// no support in MF_PHPExcel for selected sheet, therefore sheet is only selected if it is the active sheet
	...
	private function _writeSelection()
	{
		...
		$selectedCells = MF_PHPExcel_Cell::splitRange($this->_phpSheet->getSelectedCells());
		...
		list($colFirst, $rwFirst) = MF_PHPExcel_Cell::coordinateFromString($first);
		$colFirst = MF_PHPExcel_Cell::columnIndexFromString($colFirst) - 1; // base 0 column index
		--$rwFirst; // base 0 row index

		list($colLast, $rwLast) = MF_PHPExcel_Cell::coordinateFromString($last);
		$colLast = MF_PHPExcel_Cell::columnIndexFromString($colLast) - 1; // base 0 column index
	...
	private function _writeMergedCells()
	{
		...
		foreach ($mergeCells as $mergeCell) {
			...
			$range = MF_PHPExcel_Cell::splitRange($mergeCell);
			list($first, $last) = $range[0];
			list($firstColumn, $firstRow) = MF_PHPExcel_Cell::coordinateFromString($first);
			list($lastColumn, $lastRow) = MF_PHPExcel_Cell::coordinateFromString($last);

			$recordData .= pack('vvvv', $firstRow - 1, $lastRow - 1, MF_PHPExcel_Cell::columnIndexFromString($firstColumn) - 1, MF_PHPExcel_Cell::columnIndexFromString($lastColumn) - 1);
	...
	private function _writeRangeProtection()
	{
		foreach ($this->_phpSheet->getProtectedCells() as $range => $password) {
			...
			$recordData .= MF_PHPExcel_Shared_String::UTF8toBIFF8UnicodeLong('p' . md5($recordData));
	...
	private function _writePanes()
	{
		...
		if ($freezePane = $this->_phpSheet->getFreezePane()) {
			list($column, $row) = MF_PHPExcel_Cell::coordinateFromString($freezePane);
			$panes[0] = $row - 1;
			$panes[1] = MF_PHPExcel_Cell::columnIndexFromString($column) - 1;
		} else {
			// thaw panes
			return;
		}
	...
	private function _writeSetup()
	{
		...
		// Page orientation
		$fLandscape = ($this->_phpSheet->getPageSetup()->getOrientation() == MF_PHPExcel_Worksheet_PageSetup::ORIENTATION_LANDSCAPE) ?
			0x0 : 0x1;
	...
	private function _writeHeader()
	{
		...
		*/
		...
		if ($this->_BIFF_version == 0x0600) {
			$recordData = MF_PHPExcel_Shared_String::UTF8toBIFF8UnicodeLong($this->_phpSheet->getHeaderFooter()->getOddHeader());
			$length = strlen($recordData);
		} else {
	...
	private function _writeFooter()
	{
		...
		*/
		...
		if ($this->_BIFF_version == 0x0600) {
			$recordData = MF_PHPExcel_Shared_String::UTF8toBIFF8UnicodeLong($this->_phpSheet->getHeaderFooter()->getOddFooter());
			$length = strlen($recordData);
		} else {
	...
	private function _writeBreaks()
	{
		...
		foreach ($this->_phpSheet->getBreaks() as $cell => $breakType) {
			// Fetch coordinates
			$coordinates = MF_PHPExcel_Cell::coordinateFromString($cell);

			// Decide what to do by the type of break
			switch ($breakType) {
				case MF_PHPExcel_Worksheet::BREAK_COLUMN:
					// Add to list of vertical breaks
					$vbreaks[] = MF_PHPExcel_Cell::columnIndexFromString($coordinates[0]) - 1;
					break;

				case MF_PHPExcel_Worksheet::BREAK_ROW:
					// Add to list of horizontal breaks
					$hbreaks[] = $coordinates[1];
					break;

				case MF_PHPExcel_Worksheet::BREAK_NONE:
				default:
					// Nothing to do
					break;
			}
	...
	function _positionImage($col_start, $row_start, $x1, $y1, $width, $height)
	{
		...
		if ($x1 >= MF_PHPExcel_Shared_Excel5::sizeCol($this->_phpSheet, MF_PHPExcel_Cell::stringFromColumnIndex($col_start))) {
			$x1 = 0;
		}
		if ($y1 >= MF_PHPExcel_Shared_Excel5::sizeRow($this->_phpSheet, $row_start + 1)) {
			$y1 = 0;
		}
		...
		while ($width >= MF_PHPExcel_Shared_Excel5::sizeCol($this->_phpSheet, MF_PHPExcel_Cell::stringFromColumnIndex($col_end))) {
			$width -= MF_PHPExcel_Shared_Excel5::sizeCol($this->_phpSheet, MF_PHPExcel_Cell::stringFromColumnIndex($col_end));
			++$col_end;
		}

		// Subtract the underlying cell heights to find the end cell of the image
		while ($height >= MF_PHPExcel_Shared_Excel5::sizeRow($this->_phpSheet, $row_end + 1)) {
			$height -= MF_PHPExcel_Shared_Excel5::sizeRow($this->_phpSheet, $row_end + 1);
			++$row_end;
		}
		...
		//
		if (MF_PHPExcel_Shared_Excel5::sizeCol($this->_phpSheet, MF_PHPExcel_Cell::stringFromColumnIndex($col_start)) == 0) {
			return;
		}
		if (MF_PHPExcel_Shared_Excel5::sizeCol($this->_phpSheet, MF_PHPExcel_Cell::stringFromColumnIndex($col_end))   == 0) {
			return;
		}
		if (MF_PHPExcel_Shared_Excel5::sizeRow($this->_phpSheet, $row_start + 1) == 0) {
			return;
		}
		if (MF_PHPExcel_Shared_Excel5::sizeRow($this->_phpSheet, $row_end + 1)   == 0) {
			return;
		}

		// Convert the pixel values to the percentage value expected by Excel
		$x1 = $x1	 / MF_PHPExcel_Shared_Excel5::sizeCol($this->_phpSheet, MF_PHPExcel_Cell::stringFromColumnIndex($col_start))   * 1024;
		$y1 = $y1	 / MF_PHPExcel_Shared_Excel5::sizeRow($this->_phpSheet, $row_start + 1)   *  256;
		$x2 = $width  / MF_PHPExcel_Shared_Excel5::sizeCol($this->_phpSheet, MF_PHPExcel_Cell::stringFromColumnIndex($col_end))	 * 1024; // Distance to right side of object
		$y2 = $height / MF_PHPExcel_Shared_Excel5::sizeRow($this->_phpSheet, $row_end + 1)	 *  256; // Distance to bottom of object
	 ...
	 * Get Escher object
	 *
	 * @return MF_PHPExcel_Shared_Escher
	 */
	 ...
	 * Set Escher object
	 *
	 * @param MF_PHPExcel_Shared_Escher $pValue
	 */
	public function setEscher(MF_PHPExcel_Shared_Escher $pValue = null)
	{
		$this->_escher = $pValue;
	}
	...
	private function _writeMsoDrawing()
	{
		...
		if (isset($this->_escher)) {
			$writer = new MF_PHPExcel_Writer_Excel5_Escher($this->_escher);
			$data = $writer->close();
	...
	private function _writeDataValidity()
	{
		...
		$this->_append($header.$data);
			...
			foreach ($dataValidationCollection as $cellCoordinate => $dataValidation) {
				...
				switch ($type) {
					case MF_PHPExcel_Cell_DataValidation::TYPE_NONE:		$type = 0x00;	break;
					case MF_PHPExcel_Cell_DataValidation::TYPE_WHOLE:		$type = 0x01;	break;
					case MF_PHPExcel_Cell_DataValidation::TYPE_DECIMAL:	$type = 0x02;	break;
					case MF_PHPExcel_Cell_DataValidation::TYPE_LIST:		$type = 0x03;	break;
					case MF_PHPExcel_Cell_DataValidation::TYPE_DATE:		$type = 0x04;	break;
					case MF_PHPExcel_Cell_DataValidation::TYPE_TIME:		$type = 0x05;	break;
					case MF_PHPExcel_Cell_DataValidation::TYPE_TEXTLENGTH:	$type = 0x06;	break;
					case MF_PHPExcel_Cell_DataValidation::TYPE_CUSTOM:		$type = 0x07;	break;
				}
				$options |= $type << 0;
				...
				switch ($errorStyle) {
					case MF_PHPExcel_Cell_DataValidation::STYLE_STOP:			$errorStyle = 0x00;		break;
					case MF_PHPExcel_Cell_DataValidation::STYLE_WARNING:		$errorStyle = 0x01;		break;
					case MF_PHPExcel_Cell_DataValidation::STYLE_INFORMATION:	$errorStyle = 0x02;		break;
				}
				...
				switch ($operator) {
					case MF_PHPExcel_Cell_DataValidation::OPERATOR_BETWEEN: $operator = 0x00			;	break;
					case MF_PHPExcel_Cell_DataValidation::OPERATOR_NOTBETWEEN: $operator = 0x01		;	break;
					case MF_PHPExcel_Cell_DataValidation::OPERATOR_EQUAL: $operator = 0x02				;	break;
					case MF_PHPExcel_Cell_DataValidation::OPERATOR_NOTEQUAL: $operator = 0x03			;	break;
					case MF_PHPExcel_Cell_DataValidation::OPERATOR_GREATERTHAN: $operator = 0x04		;	break;
					case MF_PHPExcel_Cell_DataValidation::OPERATOR_LESSTHAN: $operator = 0x05			;	break;
					case MF_PHPExcel_Cell_DataValidation::OPERATOR_GREATERTHANOREQUAL: $operator = 0x06;	break;
					case MF_PHPExcel_Cell_DataValidation::OPERATOR_LESSTHANOREQUAL: $operator = 0x07	;	break;
				}
				$options |= $operator << 20;
				...
				$data .= MF_PHPExcel_Shared_String::UTF8toBIFF8UnicodeLong($promptTitle);

				// error title
				$errorTitle = $dataValidation->getErrorTitle() !== '' ?
					$dataValidation->getErrorTitle() : chr(0);
				$data .= MF_PHPExcel_Shared_String::UTF8toBIFF8UnicodeLong($errorTitle);

				// prompt text
				$prompt = $dataValidation->getPrompt() !== '' ?
					$dataValidation->getPrompt() : chr(0);
				$data .= MF_PHPExcel_Shared_String::UTF8toBIFF8UnicodeLong($prompt);

				// error text
				$error = $dataValidation->getError() !== '' ?
					$dataValidation->getError() : chr(0);
				$data .= MF_PHPExcel_Shared_String::UTF8toBIFF8UnicodeLong($error);

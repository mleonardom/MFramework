...
/**
 * MF_PHPExcel_Writer_Excel2007_ContentTypes
 *
 * @category   MF_PHPExcel
 * @package    MF_PHPExcel_Writer_Excel2007
 * @copyright  Copyright (c) 2006 - 2010 MF_PHPExcel (http://www.codeplex.com/MF_PHPExcel)
 */
class MF_PHPExcel_Writer_Excel2007_ContentTypes extends MF_PHPExcel_Writer_Excel2007_WriterPart
{
	/**
	 ...
	 * @throws 	Exception
	 */
	public function writeContentTypes(MF_PHPExcel $pMF_PHPExcel = null)
	{
		// Create XML writer
		$objWriter = null;
		...
		$objWriter->writeAttribute('xmlns', 'http://schemas.openxmlformats.org/package/2006/content-types');
			...
			for ($i = 0; $i < $sheetCount; ++$i) {
				if ($pMF_PHPExcel->getSheet($i)->getDrawingCollection()->count() > 0) {
					$this->_writeOverrideContentType(
			...
			// Comments
			for ($i = 0; $i < $sheetCount; ++$i) {
				if (count($pMF_PHPExcel->getSheet($i)->getComments()) > 0) {
					$this->_writeOverrideContentType(
			...
			for ($i = 0; $i < $mediaCount; ++$i) {
				...
				if ($this->getParentWriter()->getDrawingHashTable()->getByIndex($i) instanceof MF_PHPExcel_Worksheet_Drawing) {
					$extension 	= strtolower($this->getParentWriter()->getDrawingHashTable()->getByIndex($i)->getExtension());
					$mimeType 	= $this->_getImageMimeType( $this->getParentWriter()->getDrawingHashTable()->getByIndex($i)->getPath() );
				} else if ($this->getParentWriter()->getDrawingHashTable()->getByIndex($i) instanceof MF_PHPExcel_Worksheet_MemoryDrawing) {
			...
			$sheetCount = $pMF_PHPExcel->getSheetCount();
			for ($i = 0; $i < $sheetCount; ++$i) {
				if (count($pMF_PHPExcel->getSheet()->getHeaderFooter()->getImages()) > 0) {
					foreach ($pMF_PHPExcel->getSheet()->getHeaderFooter()->getImages() as $image) {
	...
	{
		if (MF_PHPExcel_Shared_File::file_exists($pFile)) {
	 ...
	 * @throws 	Exception
	 */
	private function _writeDefaultContentType(MF_PHPExcel_Shared_XMLWriter $objWriter = null, $pPartname = '', $pContentType = '')
	{
	 ...
	 * @throws 	Exception
	 */
	private function _writeOverrideContentType(MF_PHPExcel_Shared_XMLWriter $objWriter = null, $pPartname = '', $pContentType = '')
	{
		if ($pPartname != '' && $pContentType != '') {
			// Write content type

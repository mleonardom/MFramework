...
/**
 * MF_PHPExcel_Style_NumberFormat
 *
 * @category   MF_PHPExcel
 * @package	MF_PHPExcel_Style
 * @copyright  Copyright (c) 2006 - 2010 MF_PHPExcel (http://www.codeplex.com/MF_PHPExcel)
 */
class MF_PHPExcel_Style_NumberFormat implements MF_PHPExcel_IComparable
{
	/* Pre-defined formats */
	 ...
	 * @var string
	 */
	private $_formatCode	=	MF_PHPExcel_Style_NumberFormat::FORMAT_GENERAL;
	...
	/**
	 * Create a new MF_PHPExcel_Style_NumberFormat
	 */
	 ...
	 *
	 * <code>
	 * $objMF_PHPExcel->getActiveSheet()->getStyle('B2')->getNumberFormat()->applyFromArray(
	 *		array(
	 *			'code' => MF_PHPExcel_Style_NumberFormat::FORMAT_CURRENCY_EUR_SIMPLE
	 *		)
	 * );
	 * </code>
	 *
	 ...
	 * @return MF_PHPExcel_Style_NumberFormat
	 */
	public function setFormatCode($pValue = MF_PHPExcel_Style_NumberFormat::FORMAT_GENERAL)
	{
		if ($pValue == '') {
			$pValue = MF_PHPExcel_Style_NumberFormat::FORMAT_GENERAL;
		}
		if ($this->_isSupervisor) {
	...
	private static function fillBuiltInFormatCodes()
	{
		...
		if (is_null(self::$_builtInFormats)) {
			...
			// General
			self::$_builtInFormats[0] = MF_PHPExcel_Style_NumberFormat::FORMAT_GENERAL;
	...
	public static function toFormattedString($value = '', $format = '', $callBack = null)
	{
		...
		if (($format === MF_PHPExcel_Style_NumberFormat::FORMAT_GENERAL) || ($format === MF_PHPExcel_Style_NumberFormat::FORMAT_TEXT)) {
			return $value;
		}
		...
		if (preg_match('/^(\[\$[A-Z]*-[0-9A-F]*\])*[hmsdy]/i', $format)) { // datetime format
			...
			$dateObj = MF_PHPExcel_Shared_Date::ExcelToPHPObject($value);
			...
			if ($format === self::FORMAT_CURRENCY_EUR_SIMPLE) {
				...
				if (preg_match('/#?.*\?\/\?/', $format, $m)) {
					...
					if ($value != (int)$value) {
						...
						$GCD = MF_PHPExcel_Calculation_MathTrig::GCD($decimalPart,$decimalDivisor);
					...
					if (preg_match($number_regex, $format, $matches)) {
						...
						if ($useThousands) {
							$value = number_format(
								$value
								, strlen($right)
								, MF_PHPExcel_Shared_String::getDecimalSeparator()
								, MF_PHPExcel_Shared_String::getThousandsSeparator()
							);

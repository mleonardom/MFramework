 ...
 */
class MF_PHPExcel_Shared_JAMA_Matrix {
		...
		if (func_num_args() > 0) {
			...
			switch($match) {
				...
				case 'integer,integer':
						...
						$R = new MF_PHPExcel_Shared_JAMA_Matrix($m, $n);
						for($i = $i0; $i < $this->m; ++$i) {
							for($j = $j0; $j < $this->n; ++$j) {
				...
				case 'integer,integer,integer,integer':
						...
						$R = new MF_PHPExcel_Shared_JAMA_Matrix($m+1, $n+1);
						for($i = $i0; $i <= $iF; ++$i) {
							for($j = $j0; $j <= $jF; ++$j) {
				...
				case 'array,integer,integer':
						...
						if (count($RL) > 0) { $m = count($RL); } else { throw new Exception(self::ArgumentBoundsException); }
						if (($jF >= $j0) && ($this->n >= $jF) && ($j0 >= 0)) { $n = $jF - $j0; } else { throw new Exception(self::ArgumentBoundsException); }
						$R = new MF_PHPExcel_Shared_JAMA_Matrix($m, $n+1);
						for($i = 0; $i < $m; ++$i) {
							for($j = $j0; $j <= $jF; ++$j) {
	...
	public function checkMatrixDimensions($B = null) {
		if ($B instanceof MF_PHPExcel_Shared_JAMA_Matrix) {
	...
	public function transpose() {
		$R = new MF_PHPExcel_Shared_JAMA_Matrix($this->n, $this->m);
		for($i = 0; $i < $this->m; ++$i) {
			for($j = 0; $j < $this->n; ++$j) {
	...
	public function plus() {
		if (func_num_args() > 0) {
			...
			switch($match) {
				case 'object':
						if ($args[0] instanceof MF_PHPExcel_Shared_JAMA_Matrix) { $M = $args[0]; } else { throw new Exception(self::ArgumentTypeException); }
						break;
				case 'array':
						$M = new MF_PHPExcel_Shared_JAMA_Matrix($args[0]);
						break;
				default:
						throw new Exception(self::PolymorphicArgumentException);
						break;
			...
			for($i = 0; $i < $this->m; ++$i) {
				for($j = 0; $j < $this->n; ++$j) {
					...
					if ((is_string($this->A[$i][$j])) && (strlen($this->A[$i][$j]) > 0) && (!is_numeric($this->A[$i][$j]))) {
						...
						$validValues &= MF_PHPExcel_Shared_String::convertToNumberIfFraction($this->A[$i][$j]);
					}
					if ((is_string($value)) && (strlen($value) > 0) && (!is_numeric($value))) {
						$value = trim($value,'"');
						$validValues &= MF_PHPExcel_Shared_String::convertToNumberIfFraction($value);
					}
					if ($validValues) {
						$this->A[$i][$j] += $value;
					} else {
						$this->A[$i][$j] = MF_PHPExcel_Calculation_Functions::NaN();
					}
				}
			}
			return $this;
			...
			for($i = 0; $i < $this->m; ++$i) {
				for($j = 0; $j < $this->n; ++$j) {
					...
					if ($validValues) {
						...
						$M->set($i, $j, MF_PHPExcel_Calculation_Functions::NaN());
					}
				}
			}
			return $M;
	...
	public function times() {
		if (func_num_args() > 0) {
			...
			switch($match) {
				case 'object':
						if ($args[0] instanceof MF_PHPExcel_Shared_JAMA_Matrix) { $B = $args[0]; } else { throw new Exception(self::ArgumentTypeException); }
						if ($this->n == $B->m) {
							$C = new MF_PHPExcel_Shared_JAMA_Matrix($this->m, $B->n);
							for($j = 0; $j < $B->n; ++$j) {
								for ($k = 0; $k < $this->n; ++$k) {
									$Bcolj[$k] = $B->A[$k][$j];
								}
						...
						break;
				case 'array':
						$B = new MF_PHPExcel_Shared_JAMA_Matrix($args[0]);
						if ($this->n == $B->m) {
							$C = new MF_PHPExcel_Shared_JAMA_Matrix($this->m, $B->n);
							for($i = 0; $i < $C->m; ++$i) {
								for($j = 0; $j < $C->n; ++$j) {
						...
						break;
				case 'integer':
						$C = new MF_PHPExcel_Shared_JAMA_Matrix($this->A);
						for($i = 0; $i < $C->m; ++$i) {
							for($j = 0; $j < $C->n; ++$j) {
								$C->A[$i][$j] *= $args[0];
							}
						...
						break;
				case 'double':
						$C = new MF_PHPExcel_Shared_JAMA_Matrix($this->m, $this->n);
						for($i = 0; $i < $C->m; ++$i) {
							for($j = 0; $j < $C->n; ++$j) {
	...
	public function solve($B) {
		if ($this->m == $this->n) {
			$LU = new MF_PHPExcel_Shared_JAMA_LUDecomposition($this);
			return $LU->solve($B);
		} else {
	 ...
	 */
	public function det() {
		$L = new MF_PHPExcel_Shared_JAMA_LUDecomposition($this);
		return $L->det();
	}	//	function det()

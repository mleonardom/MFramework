 ...
 * @package    MF_PHPExcel_Reader_Excel5
...
// http://sourceforge.net/projects/MF_PHPExcelreader/
...
/**
 * MF_PHPExcel_Reader_Excel5
 *
 * This class uses {@link http://sourceforge.net/projects/MF_PHPExcelreader/parseXL}
 *
 * @category   MF_PHPExcel
 * @package    MF_PHPExcel_Reader_Excel5
 * @copyright  Copyright (c) 2006 - 2010 MF_PHPExcel (http://www.codeplex.com/MF_PHPExcel)
 */
class MF_PHPExcel_Reader_Excel5 implements MF_PHPExcel_Reader_IReader
{
	// ParseXL definitions
	 ...
	 * @return MF_PHPExcel_Reader_Excel5
	 */
	...
	/**
	 * Create a new MF_PHPExcel_Reader_Excel5 instance
	 */
	...
	public function canRead($pFilename)
	{
		...
		try {
			...
			$ole = new MF_PHPExcel_Shared_OLERead();
	...
	public function load($pFilename)
	{
		...
		// Initialisations
		$this->_MF_PHPExcel = new MF_PHPExcel;
		$this->_MF_PHPExcel->removeSheetByIndex(0); // remove 1st sheet
		if (!$this->_readDataOnly) {
			$this->_MF_PHPExcel->removeCellStyleXfByIndex(0); // remove the default style
			$this->_MF_PHPExcel->removeCellXfByIndex(0); // remove the default style
		}
		...
		if (!$this->_readDataOnly) {
			...
			foreach ($this->_MF_PHPExcel->getCellXfCollection() as $objStyle) {
		...
		if (!$this->_readDataOnly && $this->_drawingGroupData) {
			$escherWorkbook = new MF_PHPExcel_Shared_Escher();
			$reader = new MF_PHPExcel_Reader_Excel5_Escher($escherWorkbook);
			$escherWorkbook = $reader->load($this->_drawingGroupData);

			// debug Escher stream
			//$debug = new Debug_Escher(new MF_PHPExcel_Shared_Escher());
		...
		foreach ($this->_sheets as $sheet) {
			...
			// add sheet to MF_PHPExcel object
			$this->_phpSheet = $this->_MF_PHPExcel->createSheet();
			...
			if (!$this->_readDataOnly && $this->_drawingData) {
				$escherWorksheet = new MF_PHPExcel_Shared_Escher();
				$reader = new MF_PHPExcel_Reader_Excel5_Escher($escherWorksheet);
...
//
				...
				list($startColumn, $startRow) = MF_PHPExcel_Cell::coordinateFromString($spContainer->getStartCoordinates());
				list($endColumn, $endRow) = MF_PHPExcel_Cell::coordinateFromString($spContainer->getEndCoordinates());
				...
				$width = MF_PHPExcel_Shared_Excel5::getDistanceX($this->_phpSheet, $startColumn, $startOffsetX, $endColumn, $endOffsetX);
				$height = MF_PHPExcel_Shared_Excel5::getDistanceY($this->_phpSheet, $startRow, $startOffsetY, $endRow, $endOffsetY);

				// calculate offsetX and offsetY of the shape
				$offsetX = $startOffsetX * MF_PHPExcel_Shared_Excel5::sizeCol($this->_phpSheet, $startColumn) / 1024;
				$offsetY = $startOffsetY * MF_PHPExcel_Shared_Excel5::sizeRow($this->_phpSheet, $startRow) / 256;
...
//					echo 'Comment Object<br />';
				...
				case 0x08:
					...
					if ($blip = $BSE->getBlip()) {
						...
						$drawing = new MF_PHPExcel_Worksheet_MemoryDrawing();
						...
						switch ($blipType) {
							case MF_PHPExcel_Shared_Escher_DggContainer_BstoreContainer_BSE::BLIPTYPE_JPEG:
								$drawing->setRenderingFunction(MF_PHPExcel_Worksheet_MemoryDrawing::RENDERING_JPEG);
								$drawing->setMimeType(MF_PHPExcel_Worksheet_MemoryDrawing::MIMETYPE_JPEG);
								break;

							case MF_PHPExcel_Shared_Escher_DggContainer_BstoreContainer_BSE::BLIPTYPE_PNG:
								$drawing->setRenderingFunction(MF_PHPExcel_Worksheet_MemoryDrawing::RENDERING_PNG);
								$drawing->setMimeType(MF_PHPExcel_Worksheet_MemoryDrawing::MIMETYPE_PNG);
								break;
						}
			...
			if ($this->_version == self::XLS_BIFF8) {
				foreach ($this->_sharedFormulaParts as $cell => $baseCell) {
					list($column, $row) = MF_PHPExcel_Cell::coordinateFromString($cell);
					if ( !is_null($this->getReadFilter()) && $this->getReadFilter()->readCell($column, $row, $this->_phpSheet->getTitle()) ) {
						...
						$this->_phpSheet->getCell($cell)->setValueExplicit('=' . $formula, MF_PHPExcel_Cell_DataType::TYPE_FORMULA);
					}
				}
			}
		}
		...
		foreach ($this->_definedname as $definedName) {
			if ($definedName['isBuiltInName']) {
				...
				case pack('C', 0x06):
					...
					if ($docSheet = $this->_MF_PHPExcel->getSheetByName($sheetName)) {
				...
				case pack('C', 0x07):
					...
					foreach ($ranges as $range) {
						...
						if (count($explodes) == 2) {
							if ($docSheet = $this->_MF_PHPExcel->getSheetByName($explodes[0])) {
								...
								if (count($coordinateStrings) == 2) {
									list($firstColumn, $firstRow) = MF_PHPExcel_Cell::coordinateFromString($coordinateStrings[0]);
									list($lastColumn, $lastRow) = MF_PHPExcel_Cell::coordinateFromString($coordinateStrings[1]);
				...
				if (count($explodes) == 2) {
					if ($docSheet = $this->_MF_PHPExcel->getSheetByName($explodes[0])) {
						...
						$scope = ($definedName['scope'] == 0) ?
							null : $this->_MF_PHPExcel->getSheetByName($this->_sheets[$definedName['scope'] - 1]['name']);

						$this->_MF_PHPExcel->addNamedRange( new MF_PHPExcel_NamedRange((string)$definedName['name'], $docSheet, $extractedRange, $localOnly, $scope) );
					}
				}
			}
		}

		return $this->_MF_PHPExcel;
	}
		...
		for ($i = 0; $i < $countProperties; ++$i) {
			...
			switch ($type) {
				...
				case 0x1E: // null-terminated string prepended by dword string length
					...
					$value = MF_PHPExcel_Shared_String::ConvertEncoding($value, 'UTF-8', $codePage);
					$value = rtrim($value);
					break;

				case 0x40: // Filetime (64-bit value representing the number of 100-nanosecond intervals since January 1, 1601)
					// PHP-time
					$value = MF_PHPExcel_Shared_OLE::OLE2LocalDate(substr($this->_summaryInformation, $secOffset + 4 + $offset, 8));
					break;
			...
			switch ($id) {
				case 0x01:	//	Code Page
					$codePage = MF_PHPExcel_Shared_CodePage::NumberToName($value);
					break;

				case 0x02:	//	Title
					$this->_MF_PHPExcel->getProperties()->setTitle($value);
					break;

				case 0x03:	//	Subject
					$this->_MF_PHPExcel->getProperties()->setSubject($value);
					break;

				case 0x04:	//	Author (Creator)
					$this->_MF_PHPExcel->getProperties()->setCreator($value);
					break;

				case 0x05:	//	Keywords
					$this->_MF_PHPExcel->getProperties()->setKeywords($value);
					break;

				case 0x06:	//	Comments (Description)
					$this->_MF_PHPExcel->getProperties()->setDescription($value);
					break;

				case 0x07:	//	Template
					//	Not supported by MF_PHPExcel
					break;

				case 0x08:	//	Last Saved By (LastModifiedBy)
					$this->_MF_PHPExcel->getProperties()->setLastModifiedBy($value);
					break;
				...
				case 0x0C:	//	Created Date/Time
					$this->_MF_PHPExcel->getProperties()->setCreated($value);
					break;

				case 0x0D:	//	Modified Date/Time
					$this->_MF_PHPExcel->getProperties()->setModified($value);
					break;
...
//			echo 'Type is ',$type,', ';
			...
			switch ($type) {
				...
				case 0x40:	//	Filetime (64-bit value representing the number of 100-nanosecond intervals since January 1, 1601)
					// PHP-Time
					$value = MF_PHPExcel_Shared_OLE::OLE2LocalDate(substr($this->_documentSummaryInformation, $secOffset + 4 + $offset, 8));
					break;
			...
			switch ($id) {
				...
				case 0x02:	//	Category
					$this->_MF_PHPExcel->getProperties()->setCategory($value);
					break;
				...
				case 0x0E:	//	Manager
					$this->_MF_PHPExcel->getProperties()->setManager($value);
					break;

				case 0x0F:	//	Company
					$this->_MF_PHPExcel->getProperties()->setCompany($value);
					break;
...
//		$recordData = substr($this->_data, $this->_pos + 4, $length);
	...
	private function _readCodepage()
	{
		...
		$this->_codepage = MF_PHPExcel_Shared_CodePage::NumberToName($codepage);
	}
	...
	private function _readDateMode()
	{
		...
		MF_PHPExcel_Shared_Date::setExcelCalendar(MF_PHPExcel_Shared_Date::CALENDAR_WINDOWS_1900);
		if (ord($recordData{0}) == 1) {
			MF_PHPExcel_Shared_Date::setExcelCalendar(MF_PHPExcel_Shared_Date::CALENDAR_MAC_1904);
		}
	}
	...
	private function _readFont()
	{
		...
		if (!$this->_readDataOnly) {
			$objFont = new MF_PHPExcel_Style_Font();
			...
			switch ($underlineType) {
				case 0x00:
					break; // no underline
				case 0x01:
					$objFont->setUnderline(MF_PHPExcel_Style_Font::UNDERLINE_SINGLE);
					break;
				case 0x02:
					$objFont->setUnderline(MF_PHPExcel_Style_Font::UNDERLINE_DOUBLE);
					break;
				case 0x21:
					$objFont->setUnderline(MF_PHPExcel_Style_Font::UNDERLINE_SINGLEACCOUNTING);
					break;
				case 0x22:
					$objFont->setUnderline(MF_PHPExcel_Style_Font::UNDERLINE_DOUBLEACCOUNTING);
					break;
			}
	...
	private function _readXf()
	{
		...
		$objStyle = new MF_PHPExcel_Style();
		...
		if (!$this->_readDataOnly) {
			...
			} elseif (($code = MF_PHPExcel_Style_NumberFormat::builtInFormatCode($numberFormatIndex)) !== '') {
			...
			$objStyle->getProtection()->setLocked($isLocked ?
				MF_PHPExcel_Style_Protection::PROTECTION_INHERIT : MF_PHPExcel_Style_Protection::PROTECTION_UNPROTECTED);

			// bit 1; mask 0x02; 1 = Formula is hidden
			$isHidden = (0x02 & $xfTypeProt) >> 1;
			$objStyle->getProtection()->setHidden($isHidden ?
				MF_PHPExcel_Style_Protection::PROTECTION_PROTECTED : MF_PHPExcel_Style_Protection::PROTECTION_UNPROTECTED);
			...
			switch ($horAlign) {
				case 0:
					$objStyle->getAlignment()->setHorizontal(MF_PHPExcel_Style_Alignment::HORIZONTAL_GENERAL);
					break;
				case 1:
					$objStyle->getAlignment()->setHorizontal(MF_PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
					break;
				case 2:
					$objStyle->getAlignment()->setHorizontal(MF_PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
					break;
				case 3:
					$objStyle->getAlignment()->setHorizontal(MF_PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
					break;
				case 5:
					$objStyle->getAlignment()->setHorizontal(MF_PHPExcel_Style_Alignment::HORIZONTAL_JUSTIFY);
					break;
				case 6:
					$objStyle->getAlignment()->setHorizontal(MF_PHPExcel_Style_Alignment::HORIZONTAL_CENTER_CONTINUOUS);
					break;
			}
			...
			switch ($vertAlign) {
				case 0:
					$objStyle->getAlignment()->setVertical(MF_PHPExcel_Style_Alignment::VERTICAL_TOP);
					break;
				case 1:
					$objStyle->getAlignment()->setVertical(MF_PHPExcel_Style_Alignment::VERTICAL_CENTER);
					break;
				case 2:
					$objStyle->getAlignment()->setVertical(MF_PHPExcel_Style_Alignment::VERTICAL_BOTTOM);
					break;
				case 3:
					$objStyle->getAlignment()->setVertical(MF_PHPExcel_Style_Alignment::VERTICAL_JUSTIFY);
					break;
			}
			...
			if ($this->_version == self::XLS_BIFF8) {
				...
				// offset: 10; size: 4; Cell border lines and background area
					...
					if ($diagonalUp == false && $diagonalDown == false) {
						$objStyle->getBorders()->setDiagonalDirection(MF_PHPExcel_Style_Borders::DIAGONAL_NONE);
					} elseif ($diagonalUp == true && $diagonalDown == false) {
						$objStyle->getBorders()->setDiagonalDirection(MF_PHPExcel_Style_Borders::DIAGONAL_UP);
					} elseif ($diagonalUp == false && $diagonalDown == true) {
						$objStyle->getBorders()->setDiagonalDirection(MF_PHPExcel_Style_Borders::DIAGONAL_DOWN);
					} elseif ($diagonalUp == true && $diagonalDown == true) {
						$objStyle->getBorders()->setDiagonalDirection(MF_PHPExcel_Style_Borders::DIAGONAL_BOTH);
					}
			...
			if ($isCellStyleXf) {
				...
				if ($this->_xfIndex == 0) {
					$this->_MF_PHPExcel->addCellStyleXf($objStyle);
					$this->_mapCellStyleXfIndex[$this->_xfIndex] = 0;
				}
			} else {
				// we read all cell XF records
				$this->_MF_PHPExcel->addCellXf($objStyle);
				$this->_mapCellXfIndex[$this->_xfIndex] = count($this->_MF_PHPExcel->getCellXfCollection()) - 1;
			}
				...
				switch ($extType) {
					case 4:		// fill start color
						...
						if ($xclfType == 2) {
							...
							if ( isset($this->_mapCellXfIndex[$ixfe]) ) {
								$fill = $this->_MF_PHPExcel->getCellXfByIndex($this->_mapCellXfIndex[$ixfe])->getFill();
					...
					case 7:		// border color top
						...
						if ($xclfType == 2) {
							...
							// modify the relevant style property
							if ( isset($this->_mapCellXfIndex[$ixfe]) ) {
								$top = $this->_MF_PHPExcel->getCellXfByIndex($this->_mapCellXfIndex[$ixfe])->getBorders()->getTop();
					...
					case 8:		// border color bottom
						...
						if ($xclfType == 2) {
							...
							// modify the relevant style property
							if ( isset($this->_mapCellXfIndex[$ixfe]) ) {
								$bottom = $this->_MF_PHPExcel->getCellXfByIndex($this->_mapCellXfIndex[$ixfe])->getBorders()->getBottom();
					...
					case 9:		// border color left
						...
						if ($xclfType == 2) {
							...
							// modify the relevant style property
							if ( isset($this->_mapCellXfIndex[$ixfe]) ) {
								$left = $this->_MF_PHPExcel->getCellXfByIndex($this->_mapCellXfIndex[$ixfe])->getBorders()->getLeft();
					...
					case 10:		// border color right
						...
						if ($xclfType == 2) {
							...
							// modify the relevant style property
							if ( isset($this->_mapCellXfIndex[$ixfe]) ) {
								$right = $this->_MF_PHPExcel->getCellXfByIndex($this->_mapCellXfIndex[$ixfe])->getBorders()->getRight();
					...
					case 11:		// border color diagonal
						...
						if ($xclfType == 2) {
							...
							// modify the relevant style property
							if ( isset($this->_mapCellXfIndex[$ixfe]) ) {
								$diagonal = $this->_MF_PHPExcel->getCellXfByIndex($this->_mapCellXfIndex[$ixfe])->getBorders()->getDiagonal();
					...
					case 13:	// font color
						...
						if ($xclfType == 2) {
							...
							// modify the relevant style property
							if ( isset($this->_mapCellXfIndex[$ixfe]) ) {
								$font = $this->_MF_PHPExcel->getCellXfByIndex($this->_mapCellXfIndex[$ixfe])->getFont();
	...
	private function _readStyle()
	{
		...
		if (!$this->_readDataOnly) {
			...
			if ($isBuiltIn) {
				...
				// user-defined; not supported by MF_PHPExcel
			}
		}
	}
	...
	private function _readSheet()
	{
		...
		switch (ord($recordData{4})) {
			case 0x00: $sheetState = MF_PHPExcel_Worksheet::SHEETSTATE_VISIBLE;    break;
			case 0x01: $sheetState = MF_PHPExcel_Worksheet::SHEETSTATE_HIDDEN;     break;
			case 0x02: $sheetState = MF_PHPExcel_Worksheet::SHEETSTATE_VERYHIDDEN; break;
			default: $sheetState = MF_PHPExcel_Worksheet::SHEETSTATE_VISIBLE;      break;
		}
	...
	private function _readHorizontalPageBreaks()
	{
		...
		if ($this->_version == self::XLS_BIFF8 && !$this->_readDataOnly) {
			...
			for ($i = 0; $i < $nm; ++$i) {
				...
				$this->_phpSheet->setBreakByColumnAndRow($cf, $r, MF_PHPExcel_Worksheet::BREAK_ROW);
			}
		}
	}
	...
	private function _readVerticalPageBreaks()
	{
		...
		if ($this->_version == self::XLS_BIFF8 && !$this->_readDataOnly) {
			...
			for ($i = 0; $i < $nm; ++$i) {
				...
				$this->_phpSheet->setBreakByColumnAndRow($c, $rf, MF_PHPExcel_Worksheet::BREAK_COLUMN);
			}
		}
	}
	...
	private function _readPageSetup()
	{
		...
		if (!$this->_readDataOnly) {
			...
			if (!$isNotInit) {
				...
				switch ($isPortrait) {
				case 0: $this->_phpSheet->getPageSetup()->setOrientation(MF_PHPExcel_Worksheet_PageSetup::ORIENTATION_LANDSCAPE); break;
				case 1: $this->_phpSheet->getPageSetup()->setOrientation(MF_PHPExcel_Worksheet_PageSetup::ORIENTATION_PORTRAIT); break;
				}
	...
	private function _readRk()
	{
		...
		$columnString = MF_PHPExcel_Cell::stringFromColumnIndex($column);
		...
		if ( !is_null($this->getReadFilter()) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->_phpSheet->getTitle()) ) {
			...
			// add cell
			$cell->setValueExplicit($numValue, MF_PHPExcel_Cell_DataType::TYPE_NUMERIC);
		}
	}
	...
	private function _readLabelSst()
	{
		...
		if ( !is_null($this->getReadFilter()) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->_phpSheet->getTitle()) ) {
			...
			if (($fmtRuns = $this->_sst[$index]['fmtRuns']) && !$this->_readDataOnly) {
				...
				$richText = new MF_PHPExcel_RichText();
				$charPos = 0;
				$sstCount = count($this->_sst[$index]['fmtRuns']);
				for ($i = 0; $i <= $sstCount; ++$i) {
					if (isset($fmtRuns[$i])) {
						$text = MF_PHPExcel_Shared_String::Substring($this->_sst[$index]['value'], $charPos, $fmtRuns[$i]['charPos'] - $charPos);
						$charPos = $fmtRuns[$i]['charPos'];
					} else {
						$text = MF_PHPExcel_Shared_String::Substring($this->_sst[$index]['value'], $charPos, MF_PHPExcel_Shared_String::CountCharacters($this->_sst[$index]['value']));
					}

					if (MF_PHPExcel_Shared_String::CountCharacters($text) > 0) {
				...
				$cell = $this->_phpSheet->getCell($columnString . ($row + 1));
				$cell->setValueExplicit($richText, MF_PHPExcel_Cell_DataType::TYPE_STRING);
			} else {
				$cell = $this->_phpSheet->getCell($columnString . ($row + 1));
				$cell->setValueExplicit($this->_sst[$index]['value'], MF_PHPExcel_Cell_DataType::TYPE_STRING);
			}
	...
	private function _readMulRk()
	{
		...
		for ($i = 0; $i < $columns; ++$i) {
			$columnString = MF_PHPExcel_Cell::stringFromColumnIndex($colFirst + $i);
	...
	private function _readFormula()
	{
		...
		if ($isPartOfSharedFormula) {
			...
			$this->_baseCell = MF_PHPExcel_Cell::stringFromColumnIndex($baseCol). ($baseRow + 1);
		}
		...
		if ( !is_null($this->getReadFilter()) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->_phpSheet->getTitle()) ) {
			...
			if ( (ord($recordData{6}) == 0)
				...
				$dataType = MF_PHPExcel_Cell_DataType::TYPE_STRING;
				...
				$dataType = MF_PHPExcel_Cell_DataType::TYPE_BOOL;
				...
				$dataType = MF_PHPExcel_Cell_DataType::TYPE_ERROR;
				...
				$dataType = MF_PHPExcel_Cell_DataType::TYPE_NULL;
				$value = '';

			} else {

				// forumla result is a number, first 14 bytes like _NUMBER record
				$dataType = MF_PHPExcel_Cell_DataType::TYPE_NUMERIC;
			...
			if (!$isPartOfSharedFormula) {
				...
				try {
					...
					$cell->setValueExplicit('=' . $formula, MF_PHPExcel_Cell_DataType::TYPE_FORMULA);
		...
		if ( !is_null($this->getReadFilter()) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->_phpSheet->getTitle()) ) {
			...
			switch ($isError) {
				case 0: // boolean
					...
					// add cell value
					$cell->setValueExplicit($value, MF_PHPExcel_Cell_DataType::TYPE_BOOL);
					break;
				...
				case 1: // error type
					...
					// add cell value
					$cell->setValueExplicit($value, MF_PHPExcel_Cell_DataType::TYPE_ERROR);
					break;
			}
	...
	private function _readMulBlank()
	{
		...
		if (!$this->_readDataOnly) {
			for ($i = 0; $i < $length / 2 - 3; ++$i) {
				$columnString = MF_PHPExcel_Cell::stringFromColumnIndex($fc + $i);
	...
	private function _readBlank()
	{
		...
		$columnString = MF_PHPExcel_Cell::stringFromColumnIndex($col);
	...
	private function _readWindow2()
	{
		...
		if ($isActive) {
			$this->_MF_PHPExcel->setActiveSheetIndex($this->_MF_PHPExcel->getIndex($this->_phpSheet));
		}
	}
	...
	private function _readPane()
	{
		...
		if (!$this->_readDataOnly) {
			...
			if ($this->_frozen) {
				// frozen panes
				$this->_phpSheet->freezePane(MF_PHPExcel_Cell::stringFromColumnIndex($px) . ($py + 1));
			} else {
				// unfrozen panes; split windows; not supported by MF_PHPExcel core
			}
		}
	}
	...
	private function _includeCellRangeFiltered($cellRangeAddress)
	{
		...
		if (!is_null($this->getReadFilter())) {
			...
			$rangeBoundaries = MF_PHPExcel_Cell::getRangeBoundaries($cellRangeAddress);
	...
	private function _readHyperLink()
	{
		...
		if (!$this->_readDataOnly) {
			...
			foreach (MF_PHPExcel_Cell::extractAllCellReferencesInRange($cellRange) as $coordinate) {
	...
	private function _readDataValidation()
	{
		...
		switch ($type) {
			case 0x00:	$type = MF_PHPExcel_Cell_DataValidation::TYPE_NONE;		break;
			case 0x01:	$type = MF_PHPExcel_Cell_DataValidation::TYPE_WHOLE;		break;
			case 0x02:	$type = MF_PHPExcel_Cell_DataValidation::TYPE_DECIMAL;		break;
			case 0x03:	$type = MF_PHPExcel_Cell_DataValidation::TYPE_LIST;		break;
			case 0x04:	$type = MF_PHPExcel_Cell_DataValidation::TYPE_DATE;		break;
			case 0x05:	$type = MF_PHPExcel_Cell_DataValidation::TYPE_TIME;		break;
			case 0x06:	$type = MF_PHPExcel_Cell_DataValidation::TYPE_TEXTLENGTH;	break;
			case 0x07:	$type = MF_PHPExcel_Cell_DataValidation::TYPE_CUSTOM;		break;
		}

		// bit: 4-6; mask: 0x00000070; error type
		$errorStyle = (0x00000070 & $options) >> 4;
		switch ($errorStyle) {
			case 0x00:	$errorStyle = MF_PHPExcel_Cell_DataValidation::STYLE_STOP;			break;
			case 0x01:	$errorStyle = MF_PHPExcel_Cell_DataValidation::STYLE_WARNING;		break;
			case 0x02:	$errorStyle = MF_PHPExcel_Cell_DataValidation::STYLE_INFORMATION;	break;
		}
		...
		switch ($operator) {
			case 0x00: $operator = MF_PHPExcel_Cell_DataValidation::OPERATOR_BETWEEN			;	break;
			case 0x01: $operator = MF_PHPExcel_Cell_DataValidation::OPERATOR_NOTBETWEEN		;	break;
			case 0x02: $operator = MF_PHPExcel_Cell_DataValidation::OPERATOR_EQUAL				;	break;
			case 0x03: $operator = MF_PHPExcel_Cell_DataValidation::OPERATOR_NOTEQUAL			;	break;
			case 0x04: $operator = MF_PHPExcel_Cell_DataValidation::OPERATOR_GREATERTHAN		;	break;
			case 0x05: $operator = MF_PHPExcel_Cell_DataValidation::OPERATOR_LESSTHAN			;	break;
			case 0x06: $operator = MF_PHPExcel_Cell_DataValidation::OPERATOR_GREATERTHANOREQUAL;	break;
			case 0x07: $operator = MF_PHPExcel_Cell_DataValidation::OPERATOR_LESSTHANOREQUAL	;	break;
		}
		...
		try {
			...
			if ($type == MF_PHPExcel_Cell_DataValidation::TYPE_LIST) {
		...
		foreach ($cellRangeAddresses as $cellRange) {
			...
			$stRange = MF_PHPExcel_Cell::extractAllCellReferencesInRange($stRange);
...
//			var_dump($bcBitCount);
			...
			$drawing = new MF_PHPExcel_Worksheet_Drawing();
	...
	private function _readBIFF8CellAddress($cellAddressStructure)
	{
		...
		// offset: 2; size: 2; index to column or column offset + relative flags
			...
			$column = MF_PHPExcel_Cell::stringFromColumnIndex(0x00FF & self::_GetInt2d($cellAddressStructure, 2));
	...
	{
		list($baseCol, $baseRow) = MF_PHPExcel_Cell::coordinateFromString($baseCell);
		$baseCol = MF_PHPExcel_Cell::columnIndexFromString($baseCol) - 1;
		...
		// offset: 2; size: 2; index to column or column offset + relative flags
			...
			// bit: 14; mask 0x4000; (1 = relative column index, 0 = absolute column index)
			if (!(0x4000 & self::_GetInt2d($cellAddressStructure, 2))) {
				$column = MF_PHPExcel_Cell::stringFromColumnIndex($colIndex);
				$column = '$' . $column;
			} else {
				$colIndex = ($colIndex <= 127) ? $colIndex : $colIndex - 256;
				$column = MF_PHPExcel_Cell::stringFromColumnIndex($baseCol + $colIndex);
			}
	...
	private function _readBIFF5CellRangeAddressFixed($subData)
	{
		...
		$fc = MF_PHPExcel_Cell::stringFromColumnIndex($fc);
		$lc = MF_PHPExcel_Cell::stringFromColumnIndex($lc);
	...
	private function _readBIFF8CellRangeAddress($subData)
	{
		...
		// offset: 4; size: 2; index to first column or column offset + relative flags
			...
			// bit: 7-0; mask 0x00FF; column index
			$fc = MF_PHPExcel_Cell::stringFromColumnIndex(0x00FF & self::_GetInt2d($subData, 4));
		...
		// offset: 6; size: 2; index to last column or column offset + relative flags
			...
			// bit: 7-0; mask 0x00FF; column index
			$lc = MF_PHPExcel_Cell::stringFromColumnIndex(0x00FF & self::_GetInt2d($subData, 6));
	...
	private function _readBIFF8CellRangeAddressB($subData, $baseCell = 'A1')
	{
		...
		// offset: 4; size: 2; first column with relative/absolute flags
			...
			if (!(0x4000 & self::_GetInt2d($subData, 4))) {
				...
				$fc = MF_PHPExcel_Cell::stringFromColumnIndex($fcIndex);
				$fc = '$' . $fc;
			} else {
				// column offset
				$fcIndex = ($fcIndex <= 127) ? $fcIndex : $fcIndex - 256;
				$fc = MF_PHPExcel_Cell::stringFromColumnIndex($baseCol + $fcIndex);
			}
		...
		// offset: 6; size: 2; last column with relative/absolute flags
			...
			$lc = MF_PHPExcel_Cell::stringFromColumnIndex($baseCol + $lcIndex);

			// bit: 14; mask 0x4000; (1 = relative column index, 0 = absolute column index)
			if (!(0x4000 & self::_GetInt2d($subData, 6))) {
				// absolute column index
				$lc = MF_PHPExcel_Cell::stringFromColumnIndex($lcIndex);
				$lc = '$' . $lc;
			} else {
				// column offset
				$lcIndex = ($lcIndex <= 127) ? $lcIndex : $lcIndex - 256;
	...
	private static function _encodeUTF16($string, $compressed = '')
	{
		...
		return MF_PHPExcel_Shared_String::ConvertEncoding($string, 'UTF-8', 'UTF-16LE');
	}
	...
	{
		return MF_PHPExcel_Shared_String::ConvertEncoding($string, 'UTF-8', $this->_codepage);
	}
	...
	{
		switch ($index) {
			case 0x00: return MF_PHPExcel_Style_Border::BORDER_NONE;
			case 0x01: return MF_PHPExcel_Style_Border::BORDER_THIN;
			case 0x02: return MF_PHPExcel_Style_Border::BORDER_MEDIUM;
			case 0x03: return MF_PHPExcel_Style_Border::BORDER_DASHED;
			case 0x04: return MF_PHPExcel_Style_Border::BORDER_DOTTED;
			case 0x05: return MF_PHPExcel_Style_Border::BORDER_THICK;
			case 0x06: return MF_PHPExcel_Style_Border::BORDER_DOUBLE;
			case 0x07: return MF_PHPExcel_Style_Border::BORDER_HAIR;
			case 0x08: return MF_PHPExcel_Style_Border::BORDER_MEDIUMDASHED;
			case 0x09: return MF_PHPExcel_Style_Border::BORDER_DASHDOT;
			case 0x0A: return MF_PHPExcel_Style_Border::BORDER_MEDIUMDASHDOT;
			case 0x0B: return MF_PHPExcel_Style_Border::BORDER_DASHDOTDOT;
			case 0x0C: return MF_PHPExcel_Style_Border::BORDER_MEDIUMDASHDOTDOT;
			case 0x0D: return MF_PHPExcel_Style_Border::BORDER_SLANTDASHDOT;
			default:   return MF_PHPExcel_Style_Border::BORDER_NONE;
		}
	}
	...
	{
		switch ($index) {
			case 0x00: return MF_PHPExcel_Style_Fill::FILL_NONE;
			case 0x01: return MF_PHPExcel_Style_Fill::FILL_SOLID;
			case 0x02: return MF_PHPExcel_Style_Fill::FILL_PATTERN_MEDIUMGRAY;
			case 0x03: return MF_PHPExcel_Style_Fill::FILL_PATTERN_DARKGRAY;
			case 0x04: return MF_PHPExcel_Style_Fill::FILL_PATTERN_LIGHTGRAY;
			case 0x05: return MF_PHPExcel_Style_Fill::FILL_PATTERN_DARKHORIZONTAL;
			case 0x06: return MF_PHPExcel_Style_Fill::FILL_PATTERN_DARKVERTICAL;
			case 0x07: return MF_PHPExcel_Style_Fill::FILL_PATTERN_DARKDOWN;
			case 0x08: return MF_PHPExcel_Style_Fill::FILL_PATTERN_DARKUP;
			case 0x09: return MF_PHPExcel_Style_Fill::FILL_PATTERN_DARKGRID;
			case 0x0A: return MF_PHPExcel_Style_Fill::FILL_PATTERN_DARKTRELLIS;
			case 0x0B: return MF_PHPExcel_Style_Fill::FILL_PATTERN_LIGHTHORIZONTAL;
			case 0x0C: return MF_PHPExcel_Style_Fill::FILL_PATTERN_LIGHTVERTICAL;
			case 0x0D: return MF_PHPExcel_Style_Fill::FILL_PATTERN_LIGHTDOWN;
			case 0x0E: return MF_PHPExcel_Style_Fill::FILL_PATTERN_LIGHTUP;
			case 0x0F: return MF_PHPExcel_Style_Fill::FILL_PATTERN_LIGHTGRID;
			case 0x10: return MF_PHPExcel_Style_Fill::FILL_PATTERN_LIGHTTRELLIS;
			case 0x11: return MF_PHPExcel_Style_Fill::FILL_PATTERN_GRAY125;
			case 0x12: return MF_PHPExcel_Style_Fill::FILL_PATTERN_GRAY0625;
			default:   return MF_PHPExcel_Style_Fill::FILL_NONE;
		}
	}

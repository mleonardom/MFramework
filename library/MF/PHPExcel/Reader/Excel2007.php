...
/**
 * MF_PHPExcel_Reader_Excel2007
 *
 * @category   MF_PHPExcel
 * @package    MF_PHPExcel_Reader
 * @copyright  Copyright (c) 2006 - 2010 MF_PHPExcel (http://www.codeplex.com/MF_PHPExcel)
 */
class MF_PHPExcel_Reader_Excel2007 implements MF_PHPExcel_Reader_IReader
{
	/**
	 * Read data only?
	 *
	 ...
	 * @return MF_PHPExcel_Reader_Excel2007
	 */
	...
	/**
	 * Create a new MF_PHPExcel_Reader_Excel2007 instance
	 */
	public function __construct() {
		$this->_readFilter = new MF_PHPExcel_Reader_DefaultReadFilter();
		$this->_referenceHelper = MF_PHPExcel_ReferenceHelper::getInstance();
	}
...
//				echo 'Formula is '.$sharedFormulas[$instance]['formula'].'<br />';
				$master = MF_PHPExcel_Cell::coordinateFromString($sharedFormulas[$instance]['master']);
				$current = MF_PHPExcel_Cell::coordinateFromString($r);

				$difference = array(0, 0);
				$difference[0] = MF_PHPExcel_Cell::columnIndexFromString($current[0]) - MF_PHPExcel_Cell::columnIndexFromString($master[0]);
...
//				echo 'Adjusted Formula is '.$value.'<br />';
	...
	public function _getFromZipArchive(ZipArchive $archive, $fileName = '')
	{
		...
		$fileName = MF_PHPExcel_Shared_File::realpath($fileName);
					...
					if (is_object($xmlCore)) {
						...
						foreach ($xmlCore as $xmlProperty) {
							...
							if (isset($cellDataOfficeAttributes['name'])) {
								...
								$attributeValue = MF_PHPExcel_DocumentProperties::convertProperty($attributeValue,$attributeType);
								$attributeType = MF_PHPExcel_DocumentProperties::convertPropertyType($attributeType);
				...
				case "http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument":
					...
					if (isset($xmlStrings) && isset($xmlStrings->si)) {
						foreach ($xmlStrings->si as $val) {
							if (isset($val->t)) {
								$sharedStrings[] = MF_PHPExcel_Shared_String::ControlCharacterOOXML2PHP( (string) $val->t );
							} elseif (isset($val->r)) {
					...
					if (!$this->_readDataOnly && $xmlStyles) {
						foreach ($xmlStyles->cellXfs->xf as $xf) {
							$numFmt = MF_PHPExcel_Style_NumberFormat::FORMAT_GENERAL;
							...
							if ($xf["numFmtId"]) {
								...
								if ((int)$xf["numFmtId"] < 164) {
									$numFmt = MF_PHPExcel_Style_NumberFormat::builtInFormatCode((int)$xf["numFmtId"]);
								}
							}
								...
								foreach ($xmlSheet->cols->col as $col) {
									for ($i = intval($col["min"]) - 1; $i < intval($col["max"]); ++$i) {
										if ($col["style"] && !$this->_readDataOnly) {
											$docSheet->getColumnDimension(MF_PHPExcel_Cell::stringFromColumnIndex($i))->setXfIndex(intval($col["style"]));
										}
										if ($col["bestFit"]) {
											//$docSheet->getColumnDimension(MF_PHPExcel_Cell::stringFromColumnIndex($i))->setAutoSize(true);
										}
										if ($col["hidden"]) {
											$docSheet->getColumnDimension(MF_PHPExcel_Cell::stringFromColumnIndex($i))->setVisible(false);
										}
										if ($col["collapsed"]) {
											$docSheet->getColumnDimension(MF_PHPExcel_Cell::stringFromColumnIndex($i))->setCollapsed(true);
										}
										if ($col["outlineLevel"] > 0) {
											$docSheet->getColumnDimension(MF_PHPExcel_Cell::stringFromColumnIndex($i))->setOutlineLevel(intval($col["outlineLevel"]));
										}
										$docSheet->getColumnDimension(MF_PHPExcel_Cell::stringFromColumnIndex($i))->setWidth(floatval($col["width"]));
								...
								foreach ($xmlSheet->sheetData->row as $row) {
									...
									foreach ($row->c as $c) {
										...
										if (!is_null($this->getReadFilter())) {
											$coordinates = MF_PHPExcel_Cell::coordinateFromString($r);
	...
	//									echo 'Value is '.$value.'<br />';
										...
										// Rich text?
										if ($value instanceof MF_PHPExcel_RichText && $this->_readDataOnly) {
							...
							if (!$this->_readDataOnly && $xmlSheet && $xmlSheet->conditionalFormatting) {
								foreach ($xmlSheet->conditionalFormatting as $conditional) {
									foreach ($conditional->cfRule as $cfRule) {
										if (
											(
												(string)$cfRule["type"] == MF_PHPExcel_Style_Conditional::CONDITION_NONE ||
												(string)$cfRule["type"] == MF_PHPExcel_Style_Conditional::CONDITION_CELLIS ||
												(string)$cfRule["type"] == MF_PHPExcel_Style_Conditional::CONDITION_CONTAINSTEXT ||
												(string)$cfRule["type"] == MF_PHPExcel_Style_Conditional::CONDITION_EXPRESSION
								...
								foreach ($conditionals as $ref => $cfRules) {
									...
									foreach ($cfRules as $cfRule) {
										$objConditional = new MF_PHPExcel_Style_Conditional();
									...
									$aReferences = MF_PHPExcel_Cell::extractAllCellReferencesInRange($ref);
							...
							if ($xmlSheet && $xmlSheet->rowBreaks && $xmlSheet->rowBreaks->brk && !$this->_readDataOnly) {
								foreach ($xmlSheet->rowBreaks->brk as $brk) {
									if ($brk["man"]) {
										$docSheet->setBreak("A$brk[id]", MF_PHPExcel_Worksheet::BREAK_ROW);
									}
								}
							}
							if ($xmlSheet && $xmlSheet->colBreaks && $xmlSheet->colBreaks->brk && !$this->_readDataOnly) {
								foreach ($xmlSheet->colBreaks->brk as $brk) {
									if ($brk["man"]) {
										$docSheet->setBreak(MF_PHPExcel_Cell::stringFromColumnIndex($brk["id"]) . "1", MF_PHPExcel_Worksheet::BREAK_COLUMN);
									}
								}
							}
							...
							if ($xmlSheet && $xmlSheet->dataValidations && !$this->_readDataOnly) {
								foreach ($xmlSheet->dataValidations->dataValidation as $dataValidation) {
									...
									foreach($rangeSet as $range) {
										...
										$aReferences = MF_PHPExcel_Cell::extractAllCellReferencesInRange($stRange);
										foreach ($aReferences as $reference) {
											// Create validation
							...
							if (!$this->_readDataOnly) {
								...
								if ($xmlSheet && $xmlSheet->hyperlinks) {
									foreach ($xmlSheet->hyperlinks->hyperlink as $hyperlink) {
										...
										foreach (MF_PHPExcel_Cell::extractAllCellReferencesInRange($hyperlink['ref']) as $cellReference) {
							...
							if (!$this->_readDataOnly) {
								...
								foreach ($comments as $relName => $relPath) {
									// Load comments file
									$relPath = MF_PHPExcel_Shared_File::realpath(dirname("$dir/$fileWorksheet") . "/" . $relPath);
									...
									if ($zip->locateName(dirname("$dir/$fileWorksheet") . "/_rels/" . basename($fileWorksheet) . ".rels")) {
										...
										if ($vmlRelationship != '') {
											...
											foreach ($shapes as $shape) {
												...
												$hfImages[ (string)$shape['id'] ] = new MF_PHPExcel_Worksheet_HeaderFooterDrawing();
										...
										if ($xmlDrawing->oneCellAnchor) {
											foreach ($xmlDrawing->oneCellAnchor as $oneCellAnchor) {
												if ($oneCellAnchor->pic->blipFill) {
													...
													$objDrawing = new MF_PHPExcel_Worksheet_Drawing;
													...
													$objDrawing->setCoordinates(MF_PHPExcel_Cell::stringFromColumnIndex($oneCellAnchor->from->col) . ($oneCellAnchor->from->row + 1));
													$objDrawing->setOffsetX(MF_PHPExcel_Shared_Drawing::EMUToPixels($oneCellAnchor->from->colOff));
													$objDrawing->setOffsetY(MF_PHPExcel_Shared_Drawing::EMUToPixels($oneCellAnchor->from->rowOff));
													$objDrawing->setResizeProportional(false);
													$objDrawing->setWidth(MF_PHPExcel_Shared_Drawing::EMUToPixels(self::array_item($oneCellAnchor->ext->attributes(), "cx")));
													$objDrawing->setHeight(MF_PHPExcel_Shared_Drawing::EMUToPixels(self::array_item($oneCellAnchor->ext->attributes(), "cy")));
													if ($xfrm) {
														$objDrawing->setRotation(MF_PHPExcel_Shared_Drawing::angleToDegrees(self::array_item($xfrm->attributes(), "rot")));
													}
													if ($outerShdw) {
														$shadow = $objDrawing->getShadow();
														$shadow->setVisible(true);
														$shadow->setBlurRadius(MF_PHPExcel_Shared_Drawing::EMUTopixels(self::array_item($outerShdw->attributes(), "blurRad")));
														$shadow->setDistance(MF_PHPExcel_Shared_Drawing::EMUTopixels(self::array_item($outerShdw->attributes(), "dist")));
														$shadow->setDirection(MF_PHPExcel_Shared_Drawing::angleToDegrees(self::array_item($outerShdw->attributes(), "dir")));
										...
										if ($xmlDrawing->twoCellAnchor) {
											foreach ($xmlDrawing->twoCellAnchor as $twoCellAnchor) {
												if ($twoCellAnchor->pic->blipFill) {
													...
													$objDrawing->setPath("zip://$pFilename#" . $images[(string) self::array_item($blip->attributes("http://schemas.openxmlformats.org/officeDocument/2006/relationships"), "embed")], false);
													$objDrawing->setCoordinates(MF_PHPExcel_Cell::stringFromColumnIndex($twoCellAnchor->from->col) . ($twoCellAnchor->from->row + 1));
													$objDrawing->setOffsetX(MF_PHPExcel_Shared_Drawing::EMUToPixels($twoCellAnchor->from->colOff));
													$objDrawing->setOffsetY(MF_PHPExcel_Shared_Drawing::EMUToPixels($twoCellAnchor->from->rowOff));
													$objDrawing->setResizeProportional(false);

													$objDrawing->setWidth(MF_PHPExcel_Shared_Drawing::EMUToPixels(self::array_item($xfrm->ext->attributes(), "cx")));
													$objDrawing->setHeight(MF_PHPExcel_Shared_Drawing::EMUToPixels(self::array_item($xfrm->ext->attributes(), "cy")));
							...
							foreach ($xmlWorkbook->definedNames->definedName as $definedName) {
								...
								if ((string)$definedName['localSheetId'] != '') {
										...
										default:
											...
											if (count($range) == 2) {
												...
												if ($worksheet = $docSheet->getParent()->getSheetByName($range[0])) {
													...
													$excel->addNamedRange( new MF_PHPExcel_NamedRange((string)$definedName['name'], $worksheet, $extractedRange, true, $scope) );
												}
											}
											break;
									}
									...
									if (strpos( (string)$definedName, '!' ) !== false) {
										// Extract sheet name
										$extractedSheetName = MF_PHPExcel_Worksheet::extractSheetTitle( (string)$definedName, true );
									...
									if (!is_null($locatedSheet)) {
										$excel->addNamedRange( new MF_PHPExcel_NamedRange((string)$definedName['name'], $locatedSheet, $extractedRange, false) );
									}
								}
							}
						}
	...
	private static function _readColor($color) {
		if (isset($color["rgb"])) {
			...
			return MF_PHPExcel_Style_Color::indexedColor($color["indexed"])->getARGB();
		}
	}
	...
	private static function _readStyle($docStyle, $style) {
		...
		if (isset($style->font)) {
			...
			if (isset($style->font->u) && !isset($style->font->u["val"])) {
				$docStyle->getFont()->setUnderline(MF_PHPExcel_Style_Font::UNDERLINE_SINGLE);
		...
		if (isset($style->border)) {
			...
			if ($diagonalUp == false && $diagonalDown == false) {
				$docStyle->getBorders()->setDiagonalDirection(MF_PHPExcel_Style_Borders::DIAGONAL_NONE);
			} elseif ($diagonalUp == true && $diagonalDown == false) {
				$docStyle->getBorders()->setDiagonalDirection(MF_PHPExcel_Style_Borders::DIAGONAL_UP);
			} elseif ($diagonalUp == false && $diagonalDown == true) {
				$docStyle->getBorders()->setDiagonalDirection(MF_PHPExcel_Style_Borders::DIAGONAL_DOWN);
			} elseif ($diagonalUp == true && $diagonalDown == true) {
				$docStyle->getBorders()->setDiagonalDirection(MF_PHPExcel_Style_Borders::DIAGONAL_BOTH);
			}
		...
		if (isset($style->protection)) {
			if (isset($style->protection['locked'])) {
				if ((string)$style->protection['locked'] == 'true') {
					$docStyle->getProtection()->setLocked(MF_PHPExcel_Style_Protection::PROTECTION_PROTECTED);
				} else {
					$docStyle->getProtection()->setLocked(MF_PHPExcel_Style_Protection::PROTECTION_UNPROTECTED);
				}
			}

			if (isset($style->protection['hidden'])) {
				if ((string)$style->protection['hidden'] == 'true') {
					$docStyle->getProtection()->setHidden(MF_PHPExcel_Style_Protection::PROTECTION_PROTECTED);
				} else {
					$docStyle->getProtection()->setHidden(MF_PHPExcel_Style_Protection::PROTECTION_UNPROTECTED);
				}
			}
		}
	}
	...
	private function _parseRichText($is = null) {
		$value = new MF_PHPExcel_RichText();

		if (isset($is->t)) {
			$value->createText( MF_PHPExcel_Shared_String::ControlCharacterOOXML2PHP( (string) $is->t ) );
		} else {
			foreach ($is->r as $run) {
				if (!isset($run->rPr)) {
					$objText = $value->createText( MF_PHPExcel_Shared_String::ControlCharacterOOXML2PHP( (string) $run->t ) );

				} else {
					$objText = $value->createTextRun( MF_PHPExcel_Shared_String::ControlCharacterOOXML2PHP( (string) $run->t ) );
					...
					if (isset($run->rPr->color)) {
						$objText->getFont()->setColor( new MF_PHPExcel_Style_Color( self::_readColor($run->rPr->color) ) );
					}
					...
					if (isset($run->rPr->u) && !isset($run->rPr->u["val"])) {
						$objText->getFont()->setUnderline(MF_PHPExcel_Style_Font::UNDERLINE_SINGLE);
	...
	private static function toCSSArray($style) {
		...
		foreach ($temp as $item) {
			...
			if (strpos($item[1], 'pt') !== false) {
				...
				$item[1] = MF_PHPExcel_Shared_Font::fontSizeToPixels($item[1]);
			}
			if (strpos($item[1], 'in') !== false) {
				$item[1] = str_replace('in', '', $item[1]);
				$item[1] = MF_PHPExcel_Shared_Font::inchSizeToPixels($item[1]);
			}
			if (strpos($item[1], 'cm') !== false) {
				$item[1] = str_replace('cm', '', $item[1]);
				$item[1] = MF_PHPExcel_Shared_Font::centimeterSizeToPixels($item[1]);
			}

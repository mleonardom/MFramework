...
/**
 * MF_PHPExcel_Reader_SYLK
 *
 * @category   MF_PHPExcel
 * @package    MF_PHPExcel_Reader
 * @copyright  Copyright (c) 2006 - 2010 MF_PHPExcel (http://www.codeplex.com/MF_PHPExcel)
 */
class MF_PHPExcel_Reader_SYLK implements MF_PHPExcel_Reader_IReader
{
	/**
	 * Input encoding
	 *
	...
	/**
	 * Create a new MF_PHPExcel_Reader_SYLK
	 */
	...
	public function loadIntoExisting($pFilename, MF_PHPExcel $objMF_PHPExcel)
	{
		...
		while (($rowData = fgets($fileHandle)) !== FALSE) {
			...
			$rowData = MF_PHPExcel_Shared_String::SYLKtoUTF8($rowData);
									...
									for ($i=0;$i<strlen($styleSettings);++$i) {
										switch ($styleSettings{$i}) {
											...
											case 'D' :	$formatArray['font']['bold'] = true;
														break;
											case 'T' :	$formatArray['borders']['top']['style'] = MF_PHPExcel_Style_Border::BORDER_THIN;
														break;
											case 'B' :	$formatArray['borders']['bottom']['style'] = MF_PHPExcel_Style_Border::BORDER_THIN;
														break;
											case 'L' :	$formatArray['borders']['left']['style'] = MF_PHPExcel_Style_Border::BORDER_THIN;
														break;
											case 'R' :	$formatArray['borders']['right']['style'] = MF_PHPExcel_Style_Border::BORDER_THIN;
														break;
										}
									}
									break;
			...
			//	Read cell value data
				...
				$columnLetter = MF_PHPExcel_Cell::stringFromColumnIndex($column-1);
				$cellData = MF_PHPExcel_Calculation::_unwrapResult($cellData);

				// Set cell value
				$objMF_PHPExcel->getActiveSheet()->getCell($columnLetter.$row)->setValue(($hasCalculatedValue) ? $cellDataFormula : $cellData);
				if ($hasCalculatedValue) {
					$cellData = MF_PHPExcel_Calculation::_unwrapResult($cellData);
					$objMF_PHPExcel->getActiveSheet()->getCell($columnLetter.$row)->setCalculatedValue($cellData);
				}
			//	Read cell formatting
									...
									for ($i=0;$i<strlen($styleSettings);++$i) {
										switch ($styleSettings{$i}) {
											...
											case 'D' :	$styleData['font']['bold'] = true;
														break;
											case 'T' :	$styleData['borders']['top']['style'] = MF_PHPExcel_Style_Border::BORDER_THIN;
														break;
											case 'B' :	$styleData['borders']['bottom']['style'] = MF_PHPExcel_Style_Border::BORDER_THIN;
														break;
											case 'L' :	$styleData['borders']['left']['style'] = MF_PHPExcel_Style_Border::BORDER_THIN;
														break;
											case 'R' :	$styleData['borders']['right']['style'] = MF_PHPExcel_Style_Border::BORDER_THIN;
														break;
										}
									}
									break;
				...
				if (($formatStyle > '') && ($column > '') && ($row > '')) {
					$columnLetter = MF_PHPExcel_Cell::stringFromColumnIndex($column-1);
					$objMF_PHPExcel->getActiveSheet()->getStyle($columnLetter.$row)->applyFromArray($this->_formats[$formatStyle]);
				}
				if ((count($styleData) > 0) && ($column > '') && ($row > '')) {
					$columnLetter = MF_PHPExcel_Cell::stringFromColumnIndex($column-1);
					$objMF_PHPExcel->getActiveSheet()->getStyle($columnLetter.$row)->applyFromArray($styleData);
				}
				if ($columnWidth > '') {
					if ($startCol == $endCol) {
						$startCol = MF_PHPExcel_Cell::stringFromColumnIndex($startCol-1);
						$objMF_PHPExcel->getActiveSheet()->getColumnDimension($startCol)->setWidth($columnWidth);
					} else {
						$startCol = MF_PHPExcel_Cell::stringFromColumnIndex($startCol-1);
						$endCol = MF_PHPExcel_Cell::stringFromColumnIndex($endCol-1);
						$objMF_PHPExcel->getActiveSheet()->getColumnDimension($startCol)->setWidth($columnWidth);
						do {
							$objMF_PHPExcel->getActiveSheet()->getColumnDimension(++$startCol)->setWidth($columnWidth);
	 ...
	 * @return MF_PHPExcel_Reader_SYLK
	 */

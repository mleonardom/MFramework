...
/**
 * MF_PHPExcel_Reader_Excel2003XML
 *
 * @category   MF_PHPExcel
 * @package    MF_PHPExcel_Reader
 * @copyright  Copyright (c) 2006 - 2010 MF_PHPExcel (http://www.codeplex.com/MF_PHPExcel)
 */
class MF_PHPExcel_Reader_Excel2003XML implements MF_PHPExcel_Reader_IReader
{
	/**
	 * Read data only?
	 *
	 ...
	 * @return MF_PHPExcel_Reader_Excel2003XML
	 */
	...
	/**
	 * Create a new MF_PHPExcel_Reader_Excel2003XML
	 */
...
//
	...
	public function loadIntoExisting($pFilename, MF_PHPExcel $objMF_PHPExcel)
	{
		...
		$underlineStyles = array (
				MF_PHPExcel_Style_Font::UNDERLINE_NONE,
				MF_PHPExcel_Style_Font::UNDERLINE_DOUBLE,
				MF_PHPExcel_Style_Font::UNDERLINE_DOUBLEACCOUNTING,
				MF_PHPExcel_Style_Font::UNDERLINE_SINGLE,
				MF_PHPExcel_Style_Font::UNDERLINE_SINGLEACCOUNTING
			);
		$verticalAlignmentStyles = array (
				MF_PHPExcel_Style_Alignment::VERTICAL_BOTTOM,
				MF_PHPExcel_Style_Alignment::VERTICAL_TOP,
				MF_PHPExcel_Style_Alignment::VERTICAL_CENTER,
				MF_PHPExcel_Style_Alignment::VERTICAL_JUSTIFY
			);
		$horizontalAlignmentStyles = array (
				MF_PHPExcel_Style_Alignment::HORIZONTAL_GENERAL,
				MF_PHPExcel_Style_Alignment::HORIZONTAL_LEFT,
				MF_PHPExcel_Style_Alignment::HORIZONTAL_RIGHT,
				MF_PHPExcel_Style_Alignment::HORIZONTAL_CENTER,
				MF_PHPExcel_Style_Alignment::HORIZONTAL_CENTER_CONTINUOUS,
				MF_PHPExcel_Style_Alignment::HORIZONTAL_JUSTIFY
			);
		...
		if (isset($xml->CustomDocumentProperties)) {
			foreach($xml->CustomDocumentProperties[0] as $propertyName => $propertyValue) {
				...
				$propertyName = preg_replace_callback('/_x([0-9a-z]{4})_/','MF_PHPExcel_Reader_Excel2003XML::_hex2str',$propertyName);
				$propertyType = MF_PHPExcel_DocumentProperties::PROPERTY_TYPE_UNKNOWN;
				switch((string) $propertyAttributes) {
					case 'string' :
						$propertyType = MF_PHPExcel_DocumentProperties::PROPERTY_TYPE_STRING;
						$propertyValue = trim($propertyValue);
						break;
					case 'boolean' :
						$propertyType = MF_PHPExcel_DocumentProperties::PROPERTY_TYPE_BOOLEAN;
						$propertyValue = (bool) $propertyValue;
						break;
					case 'integer' :
						$propertyType = MF_PHPExcel_DocumentProperties::PROPERTY_TYPE_INTEGER;
						$propertyValue = intval($propertyValue);
						break;
					case 'float' :
						$propertyType = MF_PHPExcel_DocumentProperties::PROPERTY_TYPE_FLOAT;
						$propertyValue = floatval($propertyValue);
						break;
					case 'dateTime.tz' :
						$propertyType = MF_PHPExcel_DocumentProperties::PROPERTY_TYPE_DATE;
...
//									echo $borderStyleKey.' = '.$borderStyleValue.'<br />';
									switch ($borderStyleKey) {
										case 'LineStyle' :
												$thisBorder['style'] = MF_PHPExcel_Style_Border::BORDER_MEDIUM;
		...
		foreach($xml->Worksheet as $worksheet) {
			...
			foreach($worksheet->Table->Column as $columnData) {
				...
				if (isset($columnData_ss['Index'])) {
					$columnID = MF_PHPExcel_Cell::stringFromColumnIndex($columnData_ss['Index']-1);
				}
				if (isset($columnData_ss['Width'])) {
					$columnWidth = $columnData_ss['Width'];
//					echo '<b>Setting column width for '.$columnID.' to '.$columnWidth.'</b><br />';
					$objMF_PHPExcel->getActiveSheet()->getColumnDimension($columnID)->setWidth($columnWidth / 5.4);
				}
				++$columnID;
			}
...
//					echo '<b>Setting row height to '.$rowHeight.'</b><br />';
					$objMF_PHPExcel->getActiveSheet()->getRowDimension($rowID)->setRowHeight($rowHeight);
				}
				$columnID = 'A';
				foreach($rowData->Cell as $cell) {
					...
					if (isset($cell_ss['Index'])) {
						$columnID = MF_PHPExcel_Cell::stringFromColumnIndex($cell_ss['Index']-1);
					}
					...
					if ((isset($cell_ss['MergeAcross'])) || (isset($cell_ss['MergeDown']))) {
						...
						if (isset($cell_ss['MergeAcross'])) {
							$columnTo = MF_PHPExcel_Cell::stringFromColumnIndex(MF_PHPExcel_Cell::columnIndexFromString($columnID) + $cell_ss['MergeAcross'] -1);
						}
						$rowTo = $rowID;
...
//							echo "found an array formula at ".$columnID.$rowID."<br />";
					...
					if (isset($cell->Data)) {
						...
						if (isset($cellData_ss['Type'])) {
							...
							switch ($cellDataType) {
								...
								*/
								...
								case 'DateTime' :
										$type = MF_PHPExcel_Cell_DataType::TYPE_NUMERIC;
										$cellValue = MF_PHPExcel_Shared_Date::PHPToExcel(strtotime($cellValue));
										break;
								case 'Error' :
						...
						if ($hasCalculatedValue) {
							...
							$columnNumber = MF_PHPExcel_Cell::columnIndexFromString($columnID);
							...
							foreach($temp as $key => &$value) {
								...
								if (($key % 2) == 0) {
									...
									foreach($cellReferences as $cellReference) {
										...
										$A1CellReference = MF_PHPExcel_Cell::stringFromColumnIndex($columnReference-1).$rowReference;
...
//
						$objMF_PHPExcel->getActiveSheet()->getCell($columnID.$rowID)->setValueExplicit((($hasCalculatedValue) ? $cellDataFormula : $cellValue),$type);
						if ($hasCalculatedValue) {
//							echo 'Forumla result is '.$cellValue.'<br />';
							$objMF_PHPExcel->getActiveSheet()->getCell($columnID.$rowID)->setCalculatedValue($cellValue);
						}
					}
...
//							echo '<br />';
							if (!$objMF_PHPExcel->getActiveSheet()->cellExists($columnID.$rowID)) {
								$objMF_PHPExcel->getActiveSheet()->getCell($columnID.$rowID)->setValue(NULL);
							}
							$objMF_PHPExcel->getActiveSheet()->getStyle($cellRange)->applyFromArray($this->_styles[$style]);
						}
					}
					++$columnID;
				}
